    ...
    try {
      ...
      if (testCodes.includes(targetCode)) {
        if (Date.now() > expirationDate) {
          alert("🚨 此封測體驗碼已逾期，請聯繫 Aura EyeGym 開發團隊獲取正式授權。");
          return; // 逾期直接阻擋
        }
        
        // 未逾期，直接發放權限並跳過資料庫驗證
        localStorage.setItem('aura_premium_unlocked', 'true');
        localStorage.setItem('aura_clinic_id', 'BETA-TESTER'); // 標記為測試者
        localStorage.setItem('aura_activation_code', targetCode);
        
        setIsPremiumUnlocked(true);
        setShowRedeemModal(false);
        alert("✅ 封測授權成功，歡迎進入 Aura EyeGym");
        return; // 攔截成功，提早結束函數，不執行下方 Supabase 連線
      }
          ...
          <>
            ...
            {trackingState === 'CALIBRATING' && (
              <div className="absolute inset-0 z-40 bg-black/80 flex flex-col items-center justify-center backdrop-blur-md pointer-events-auto transition-opacity duration-300">
                ...
                {/* 動態指示 UI */}
                {calibrationStatus === 'TOO_CLOSE' && <p className="text-[#ff4d4d] text-[20px] font-bold bg-[#2b1616] px-4 py-2 rounded-lg border border-[#ff4d4d]">太近了，請稍微退後 🛑</p>}
                {calibrationStatus === 'TOO_FAR' && <p className="text-[#E5B55E] text-[20px] font-bold bg-[#2a1f1a] px-4 py-2 rounded-lg border border-[#E5B55E]">太遠了，請稍微靠近 🔍</p>}
        ...
        {showRedeemModal && (
          <div className="absolute inset-0 z-[100] bg-black/80 flex items-center justify-center p-5 backdrop-blur-sm">
            <div className="bg-[#1a2233] border-2 border-[#00ffcc] p-6 rounded-2xl w-full max-w-[400px] shadow-[0_0_25px_rgba(0,255,204,0.3)] text-center">
              ...
              <p className="text-[#8b9bb4] text-[14px] mb-4">請輸入診所配發之專屬序號</p>
              <input 
                type="text" 
                placeholder="例如: VIP-8859 或 專案代碼" 
                value={redeemCode}
